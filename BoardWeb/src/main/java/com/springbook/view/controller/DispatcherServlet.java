...
public class DispatcherServlet extends HttpServlet {
	...
	private void process(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		/*
		 * //1.클라이언트의 요청 path 정보를 추출한다. String uri = request.getRequestURI(); String
		 * path = uri.substring(uri.lastIndexOf("/")); System.out.println(path);
		 * 
		 * //2.클라이언트의 요청 path에 따라 적절히 분기처리한다. if(path.equals("/login.do")) {
		 ...
		 * 
		 * if(user != null) { //세션등록 response.sendRedirect("getBoardList.do"); }else {
		 * response.sendRedirect("login.jsp"); }
		 * 
		 * }else if(path.equals("/logout.do")) { System.out.println("로그아웃처리");
		 * 
		 ...
		 * String seq = request.getParameter("seq"); BoardVO vo = new BoardVO();
		 * vo.setSeq(Integer.parseInt(seq)); BoardDAO boardDAO = new BoardDAO();
		 * boardDAO.deleteBoard(vo); response.sendRedirect("getBoardList.do");
		 * 
		 * }else if(path.equals("/getBoard.do")) { System.out.println("글 상세 조회 처리");
		 * String seq = request.getParameter("seq"); BoardVO vo = new BoardVO();
